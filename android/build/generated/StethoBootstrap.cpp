...
#define TAG "ti.stetho"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Stetho_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Stetho.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::StethoBindings::lookupGeneratedInit(
...
static void Stetho_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Stetho_getBinding)->GetFunction());
}

static void Stetho_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::StethoBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry StethoBinding = {
	"ti.stetho",
	Stetho_init,
	Stetho_dispose
};
...
Java_ti_stetho_StethoBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.stetho", &StethoBinding);
	titanium::KrollBindings::addExternalLookup(&(::StethoBindings::lookupGeneratedInit));
}
